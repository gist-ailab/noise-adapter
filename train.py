...
def train():
    ...
    parser.add_argument('--rate', '-r', default = 0.0, type=float)
    parser.add_argument('--save_path', '-s', type=str)
    args = parser.parse_args()

    config = utils.read_conf('conf/'+args.conf+'.json')
    ...
    print(dataset_path.split('/')[-2], batch_size, class_range)
    ...
    train_loader, valid_loader = utils.get_ham10000(dataset_path, batch_size, args.rate)
    ...
    for epoch in range(max_epoch):
        ...
        torch.save(model.state_dict(), '{}/last_checkpoint.pth'.format(save_path))
