...
class APTOS2019TwoLabel():
    ...
    def __getitem__(self, idx):
        sample, label = self.samples[idx]
        sample = Image.open(sample)
        clean = self.ori_labels[idx]

        if self.transform:
            sample = self.transform(sample)

        return sample, label, clean
    

class APTOS2019_valid():
    def __init__(self, root_dir, transforms=None):
        ...
        self.label_txt = os.path.join(root_dir, 'valid.csv')
        ...
        with open(self.label_txt, 'r') as f:
            ...
            for line in lines[1:]:
                ...
                if len(line) == 2:
                    ...
                    img_name = os.path.join(root_dir, 'val_images/val_images', img_name+'.png')
                    label = label.replace('\n', '')
                    label = int(label)
