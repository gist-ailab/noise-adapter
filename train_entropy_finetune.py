...
def train():
    ...
    base_path = config['save_path'] + args.base_path
    ...
    if args.net == 'resnet18':
        ...
        model.load_state_dict(torch.load(base_path + '/last.pth.tar', map_location=device)['state_dict'])
    else:
    ...
    for epoch in range(30):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            ce_loss = criterion_noreduction(outputs, pseudo_label_ema)
            # consistency_loss = softmax_entropy(outputs, outputs_ema).mean()
            loss = ce_loss.mean()# + consistency_loss
            loss.backward()            
            optimizer.step()
        ...
        print(ema_accuracy, train_accuracy)
        # print()
