...
def train():
    ...
    if args.net == 'resnet18':
        ...
        model = timm.create_model(args.net, pretrained=False, num_classes=num_classes, img_size=192)
    model.to(device)
    ...
    for epoch in range(max_epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            easy_data = inputs
            easy_label = targets #pseudo_label_ema[pseudo_label_ema == targets]
            easy_weight = inputs.size(0)/easy_data.size(0)
            
            # CE loss for Easy data Ce loss is computed for training the ViT. 
            ...
            mim_loss = 0
            for i in range(1):
                masked_easy_data = masking_image(easy_data, 0.8)
            ...
            loss = easy_weight * ce_loss.mean() + mim_loss
            loss.backward()            
            optimizer.step()
            ...
            _, predicted = outputs_easy[:len(targets)].max(1)            
