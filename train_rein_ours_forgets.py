...
def train():
    ...
    for epoch in range(max_epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ind_1_sorted = np.argsort(loss_linear.cpu().data).cuda()
                remember_rate = 1-noise_rate
                num_remember = int(remember_rate * len(ind_1_sorted))
                ind_rein_update = ind_1_sorted[:num_remember]
                # print(len(ind_rein_update), ind_rein_update)


            loss_rein = criterion(outputs, targets)[ind_rein_update]
        ...
        total = 0
        correct = 0
        valid_accuracy = utils.validation_accuracy_ours(model, valid_loader, device, 'rein')
